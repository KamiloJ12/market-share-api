...
export class CompanyService {
  ...
  obtenerDatos(): Observable<any> {
    const url = 'http://localhost:8000/api/v1/companies/';
    return this.http.get<any>(url);
  }

  obtenerDatosPorId(id:number): Observable<any> {
    const url = 'http://localhost:8000/api/v1/company/' + id;
    return this.http.get<any>(url);
  }
  ...
  actualizar(data: any): Observable<any> {
    const url = 'http://localhost:8000/api/v1/company/' + data.id;
    return this.http.put<any>(url, data);
  }

  registrarVenta(data: any) {
    const url = 'http://localhost:8000/api/v1/market-share/';
    return this.http.post<any>(url, data);
  }

  obtenerVentas(id_company: number, id_product:number) {
    const url = 'http://localhost:8000/api/v1/market-share/'+id_company+'/'+id_product;
    return this.http.get<any>(url);
  }
}
